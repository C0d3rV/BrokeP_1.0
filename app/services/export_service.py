...
def _draw_footer(canvas, doc):
    ...
    canvas.drawCentredString(
        ...
        f"Generated by BrokeP Trading System · Page {canvas.getPageNumber()}"
    )
    canvas.restoreState()
...
def export_daily_snapshot_to_excel(trades_with_marks, client, target_date: str, target_filepath: str) -> None:
    wb = Workbook()
    ws = wb.active
...
def export_daily_snapshot_to_pdf(trades_with_marks, client, target_date: str, target_filepath: str) -> None:
    doc = SimpleDocTemplate(target_filepath, pagesize=portrait(A4), leftMargin=1.5*cm, rightMargin=1.5*cm, topMargin=2*cm, bottomMargin=2*cm)
    styles = getSampleStyleSheet()
